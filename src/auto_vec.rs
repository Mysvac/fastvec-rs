...
use crate::{StackVec, utils::cold_path};

#[derive(Clone, Debug)]
pub(crate) enum InnerVec<T, const N: usize> {
    Stack(StackVec<T, N>),
    Heap(Vec<T>),
}
...
/// as it incurs additional judgment overhead.
///
/// # Example
///
/// ```
...
#[derive(Clone)]
#[repr(transparent)]
pub struct AutoVec<T, const N: usize>(pub(crate) InnerVec<T, N>);
...
impl<T, const N: usize> AutoVec<T, N> {
    ...
    pub unsafe fn from_raw_parts(ptr: *mut T, length: usize, capacity: usize) -> Self {
        ...
        unsafe { Self(InnerVec::Heap(Vec::from_raw_parts(ptr, length, capacity))) }
    }
        ...
        if capacity > N {
            match &mut self.0 {
                InnerVec::Stack(vec) => {
                    // SAFETY: capacity >= len
                    self.0 =
                        InnerVec::Heap(unsafe { vec.into_vec_with_capacity_unchecked(capacity) });
                }
            ...
            InnerVec::Stack(vec) => {
                ...
                if vec.len() < N {
                    ...
                    let mut new_vec: Vec<T> =
                        Vec::with_capacity(core::cmp::max(N << 1, crate::utils::min_cap::<T>()));
        ...
        match &mut self.0 {
            InnerVec::Stack(vec) => {
                if new_len <= N {
                    ...
                    let mut vec = unsafe { vec.into_vec_with_capacity_unchecked(new_len) };
                    vec.resize_with(new_len, f);
                    self.0 = InnerVec::Heap(vec);
                }
            }
...
impl<T: Clone, const N: usize> AutoVec<T, N> {
    ...
    pub fn from_elem(elem: T, num: usize) -> Self {
        let mut vec = Self::with_capacity(num);
        ...
        match &mut self.0 {
            InnerVec::Stack(vec) => {
                ...
                if capacity <= N {
                    ...
                    cold_path();
                    // SAFETY: capacity == new_len > len
                    let mut vec = unsafe { vec.into_vec_with_capacity_unchecked(capacity) };
                    vec.extend_from_slice(other);
                    self.0 = InnerVec::Heap(vec);
                }
            }
        ...
        match self.0 {
            ...
            InnerVec::Heap(vec) => {
                if S >= P * vec.len() {
                    ...
                    unsafe {
                        <StackVec<T, S>>::from_vec_unchecked(&mut vec.into_flattened()).into()
                    }
                } else {
...
impl<T, const N: usize> IntoIterator for AutoVec<T, N> {
    ...
    fn into_iter(self) -> Self::IntoIter {
        match self.0 {
            ...
            InnerVec::Heap(vec) => {
                cold_path();
                IntoIter::Heap(IntoIterator::into_iter(vec))
            }
        }
    }
}
